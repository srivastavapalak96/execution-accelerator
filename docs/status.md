...
| Complex remediation lane | PARTIAL | Analysis/execution scaffold exists, and the persisted complex plan now records a primary migration tactic plus actionable migration steps and concrete file/symbol targets derived from compatibility analysis, but real migration execution logic is still missing. |
...
| Observability and escalation bundles | PARTIAL | Failed runs now write JSON escalation bundles under `data/escalations/`, and CLI summaries surface bundle paths, skip reasons, and the latest workflow error; broader observability hardening is still missing. |
