...
| Typed schemas and remediation state | SHIPPED | Core workflow models and state exist, persisted remediation plans now carry more concrete complex-lane summary/rationale details, and complex scaffold execution can apply deterministic Java method-call rewrites for supported mappings. |
...
| Observability and escalation bundles | PARTIAL | Failed runs now write JSON escalation bundles under `data/escalations/`, including code-diff summaries and richer complex-plan context for tough-path failures, and CLI summaries surface bundle paths, skip reasons, changed-file summaries, aggregate diff totals, primary validation checks, and the latest workflow error; broader observability hardening is still missing. |
