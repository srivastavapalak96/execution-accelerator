...
| Observability and escalation bundles | PARTIAL | Failed runs now write JSON escalation bundles under `data/escalations/`, including code-diff summaries, latest validation status/check context, and richer complex-plan context for tough-path failures, and CLI summaries surface bundle paths, skip reasons, route rationale, changed-file summaries, aggregate diff totals, primary validation checks, and the latest workflow error; broader observability hardening is still missing. |
