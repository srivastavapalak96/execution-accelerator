...
| Observability and escalation bundles | PARTIAL | Failed runs now write JSON escalation bundles under `data/escalations/`, including richer complex-plan context for tough-path failures, and CLI summaries surface bundle paths, skip reasons, and the latest workflow error; broader observability hardening is still missing. |
