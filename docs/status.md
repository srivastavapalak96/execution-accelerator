...
| Delivery | PARTIAL | Live branch publication and PR creation now exist; Jira completion can post a comment and optionally fire a configured done transition, delivery-approved complex runs can publish ready PRs instead of remaining draft-only, and GitHub PR-create conflicts can recover the existing open PR instead of failing delivery, but richer workflow handling is still missing. |
