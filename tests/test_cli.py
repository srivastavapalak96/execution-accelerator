...
def test_main_prints_escalation_bundle_for_failed_ticket(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert "latest_error_code=validation_compile_failed" in captured.out
    assert "retry_count=0" in captured.out
    bundle_line = next(
...
def test_main_hides_stale_retry_failure_context_after_successful_delivery_retry(monkeypatch, capsys, tmp_path) -> None:
    fixture_dir = Path(__file__).parent / "fixtures"
    monkeypatch.setattr(
        "sys.argv",
        [
    ...
    assert "failure_classification=" not in captured.out
    assert "latest_error_code=" not in captured.out
    assert "retry_next_node=" not in captured.out
    assert "jira_ticket_status=done" in captured.out
...
def test_main_resumes_complex_ticket_after_approval(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert "plan_summary=Analyze org.example:legacy-json from 1.2.3 to 2.0.0 before attempting code changes. Executed bounded complex migration edits for 2 files." in captured.out
    assert "plan_rationale=Removed APIs and constructor changes suggest insulating callers behind a compatibility adapter while parser and serializer internals migrate. Current bounded execution covers 2 planned target files, 0 detected existing source files, and 2 unresolved questions." in captured.out
    ...
    assert "code_diff_total_additions=30" in captured.out
